...
class ServicesView(WidgetWrap):
    ...
    def refresh_nodes(self):
        ...
        for name, info in status['Services'].items():
            service_w = ServiceWidget(name, info)
            for unit in service_w.Units:
                ...
                try:
                    ...
                    self.table.addColumns(
                        ...
                        [
                            ('fixed', 5, Text("")),
                            Color.frame_subheader(unit_w.WorkloadInfo)
                        ],
                        force=True)
                self.update_ui_state(unit_w)

    def status_icon_state(self, agent_state):
        if agent_state == "pending":
    ...
    def update_ui_state(self, unit):
        """ Updates individual machine information
        """
        unit.PublicAddress.set_text(unit._unit['PublicAddress'])
        unit.AgentState.set_text(unit._unit['AgentState'])
        unit.Icon.set_text(self.status_icon_state(unit._unit['AgentState']))
